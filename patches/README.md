...
exactly what a GDI window still looks like. Nine sites were converted and the
border remains, so at least one more place sizes the surface from the display's
backing scale factor rather than from `RetinaScale`. **This is unfinished:** the
option is correct for D3D/Metal windows (the game) and leaves an unpainted border
on GDI windows (the Arena's launcher). Anyone taking this upstream should start
there.
